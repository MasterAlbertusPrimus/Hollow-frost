...
// ============================================================
//
// The page is loaded as index.html?level=1, index.html?level=2, etc.
// Whichever number is in the URL picks which levels/level_N.json file
// gets fetched. WORLD_DATA is not set until that fetch resolves, so
// nothing else in this file may run until initGame() is called.
...
fetch(LEVEL_FILE)
    .then(response => {
        if (!response.ok) {
            throw new Error(`Failed to load ${LEVEL_FILE} (${response.status})`);
        }
    ...
    .catch(error => {
        console.error(error);
        // Fail loudly in the UI too - a silent blank canvas with only a
        // console error is very easy to mistake for "the game is broken"
        // rather than "the level file didn't load".
        var titleEl = document.getElementById('titleText');
        var subEl = document.getElementById('subText');
        var startEl = document.getElementById('startBtn');
        if (titleEl) titleEl.textContent = 'Could not load level';
        if (subEl) {
            subEl.textContent = 'Tried to fetch ' + LEVEL_FILE + ' — ' + (error && error.message ? error.message : error);
...
// ============================================================
// The game itself. Not run until WORLD_DATA has been fetched.
// ============================================================
...
function initGame() {
    ...
    function updateWorldDataOutput() {
        ...
        output.solids = WORLD_DATA.solids.map(function(s) {
            return {
                ...
                w: Math.round(s.w),
                h: Math.round(s.h),
                rot: Math.round(s.rot || 0)
            };
        });

        // Pure JSON, ready to paste straight into levels/level_N.json -
        // no "var WORLD_DATA = " wrapper.
    ...
    function updateSelectedSummary() {
        ...
        if (ent.w !== undefined) {
            ...
            if (ent.rot) txt += '  rot:' + Math.round(ent.rot) + '°';
        }
    ...
    function editorMouseMove(e) {
        ...
        if (editorRotating) {
            ...
            if (!e.altKey) newRot = Math.round(newRot / 15) * 15;
    ...
    // ---- Rotated Collision Detection (SAT) ----
    ...
    function broadPhase(entity, s, extra) {
        var a = { x: entity.x, y: entity.y, w: entity.w, h: entity.h };
        var b = cbox(s);
        extra = extra || 0;
    ...
    function playerSolidMTV(entity, solid) {
        ...
        var playerBox = { x: entity.x, y: entity.y, w: entity.w, h: entity.h, rot: 0 };
    ...
    function moveAndCollide(entity, dt, world, allowDrop) {
        ...
        if (allowDrop !== false && world.platforms) {
            ...
            var currentBox = { x: entity.x, y: entity.y, w: entity.w, h: entity.h, rot: 0 };
    ...
    var collected = {};
    var lives = 3;
    var checkpoint = { x: WORLD_DATA.startPos.x, y: WORLD_DATA.startPos.y };
    var running = false;
    var won = false;
    var elapsed = 0;
    ...
    var player = null;
    var camera = { x: 0, y: 0 };
    var keys = { left: false, right: false, jumpHeld: false, dash: false, down: false };
    ...
    function freshPlayer(x, y) {
        return {
            ...
            onGround: false, onWallDir: 0, facing: 1,
            jumpsUsed: 0, dashTime: 0, dashCooldown: 0, dashDir: 1,
            coyote: 0, jumpBuffer: 0, hurtT: 0, invT: 1.2
        };
    }
    ...
    function resetEnemies() {
        enemyState = WORLD_DATA.enemies.map(function(e) {
            return {
                type: e.type, x: e.x, y: e.y, homeX: e.homeX,
                homeY: e.type === 'flyer' ? e.y : null,
                minX: e.range[0], maxX: e.range[1],
                dir: 1, speed: e.type === 'flyer' ? 42 : 32,
                vy: 0, bob: rand(0, Math.PI * 2)
            };
        });
    }
    ...
    function burst(x, y, color, n) {
        n = n || 12;
        for (var i = 0; i < n; i++) {
            var a = Math.random() * Math.PI * 2, sp = rand(20, 80);
            particles.push({
                x: x, y: y,
    ...
    // The game's name ("Hollow Frost") is fixed chrome; each LEVEL
    // supplies its own name and (optionally) its own per-room zone
    // names, straight from its own JSON - nothing hardcoded here.
    function currentZoneName(px, py) {
        var zones = WORLD_DATA.zoneNames || {};
        if (WORLD_DATA.ROOM_W && WORLD_DATA.TILE) {
            var gx = Math.floor(px / (WORLD_DATA.ROOM_W * WORLD_DATA.TILE));
            var gy = Math.floor(py / (WORLD_DATA.ROOM_H * WORLD_DATA.TILE));
            var roomName = zones[gx + ',' + gy];
            if (roomName) return roomName;
        }
        return WORLD_DATA.name || 'Hollow Frost';
    }
    ...
    // Sets up all runtime state (solids, enemies, stars, checkpoint,
    // player, camera) from the currently-loaded WORLD_DATA. There is
    // only ever one level's worth of WORLD_DATA per page load, so this
    // replaces the old "loadLevel(index, keepProgress)" array-switch.
    function initFromWorldData(keepProgress) {
        WORLD_DATA.items.forEach(function(it) { it.taken = false; });
        WORLD_DATA.crystals.forEach(function(c) { c.activated = false; });
        ...
        checkpoint = { x: WORLD_DATA.startPos.x, y: WORLD_DATA.startPos.y };
        ...
        if (!keepProgress) {
            abilities = { doubleJump: false, dash: false, wallCling: false, breakBlocks: false };
            collected = {};
            lives = 3;
            elapsed = 0;
        }
    ...
    function checkItemPickup() {
        ...
        for (var i = 0; i < WORLD_DATA.items.length; i++) {
            ...
            var ibox = { x: it.x - 8, y: it.y - 8, w: 16, h: 16 };
            if (aabb(pbox, ibox)) {
                it.taken = true;
                ...
                if (it.id.indexOf('ability_') === 0) {
                    ...
                    // Multi-level progression isn't wired up yet - every
                    // level's goal just shows this placeholder for now.
                    won = true;
                    running = false;
                    showToast('Next level coming soon!');
                    setTimeout(showComingSoon, 500);
                }
            }
        }
    }
    ...
    function checkCrystal() {
        ...
        WORLD_DATA.crystals.forEach(function(c) {
            ...
            var cbox2 = { x: c.x - 8, y: c.y - 10, w: 16, h: 20 };
            if (aabb(pbox, cbox2)) {
                c.activated = true;
                checkpoint = { x: c.x, y: c.y - 4 };
    ...
    function checkBreakables() {
        ...
        var nextBox = { x: player.x + player.vx / 60, y: player.y, w: player.w, h: player.h };
    ...
    function updateEnemies(dt) {
        for (var i = 0; i < enemyState.length; i++) {
            ...
            if (e.type === 'walker') {
                ...
                e.vy += PHYS.GRAV * dt;
                var testBox = { x: e.x, y: e.y + e.vy * dt, w: 12, h: 12 };
                var grounded = false;
                for (var s = 0; s < liveSolids.length; s++) {
    ...
    function update(dt) {
        ...
        var input = {
            ...
            jumpPressed: jumpPressedFlag, jumpHeld: keys.jumpHeld,
            dash: keys.dash, down: keys.down
        };
    ...
    function drawEditorOverlay(vw, vh) {
        ...
        var left = camera.x - vw / 2, right = camera.x + vw / 2;
        var top = camera.y - vh / 2, bottom = camera.y + vh / 2;
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (def.shape === 'point') {
                ...
                var rot = ent.rot || 0;
                var cx = ent.x + ent.w / 2, cy = ent.y + ent.h / 2;
    ...
    bindHold(btnJump, function() {
        if (!keys.jumpHeld) jumpPressedFlag = true;
        keys.jumpHeld = true;
    }, function() { keys.jumpHeld = false; });
    bindHold(btnDash, function() { keys.dash = true; }, function() {});

    // ---------- UI flow ----------
    function showComingSoon() {
        titleText.textContent = 'Next level coming soon';
        subText.classList.add('hidden');
        legendEl.classList.add('hidden');
        statLine.classList.remove('hidden');
        statLine.textContent = 'You cleared ' + (WORLD_DATA.name || 'this level') +
            ' in ' + Math.floor(elapsed) + 's';
