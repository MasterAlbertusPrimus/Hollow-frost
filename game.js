...
fetch(LEVEL_FILE)
	.then(response => {
		if (!response.ok) {
			throw new Error(`Failed to load ${LEVEL_FILE}`);
		}
	...
	.then(data => {
		...
		// Tell the existing game to start here.
		initGame();
	})
	.catch(error => {
		console.error(error);
...
function initGame() {
	...
	var ROW_IDS = {
		...
		rot: 'rowRot',
		itemId: 'rowItemId',
		crystalId: 'rowCrystalId',
		enemyType: 'rowEnemyType',
		homeX: 'rowHomeX',
		rangeMin: 'rowRangeMin',
		rangeMax: 'rowRangeMax',
		color: 'rowColor',
		broken: 'rowBroken'
	};
	...
	bindNumberField('fldX', function(e, v) {
		e.x = v;
	});
	bindNumberField('fldY', function(e, v) {
		e.y = v;
	});
	bindNumberField('fldW', function(e, v) {
		e.w = Math.max(4, v);
	});
	bindNumberField('fldH', function(e, v) {
		e.h = Math.max(4, v);
	});
	bindNumberField('fldRot', function(e, v) {
		e.rot = ((v % 360) + 360) % 360;
	});
	bindNumberField('fldHomeX', function(e, v) {
		e.homeX = v;
	});
	bindNumberField('fldRangeMin', function(e, v) {
		e.range[0] = v;
	});
	bindNumberField('fldRangeMax', function(e, v) {
		e.range[1] = v;
	});
